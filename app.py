...
def search_transactions():
    if request.method == "POST":
        # Retrieve form data
        min_amount = float(request.form.get('min_amount', 0.0))
        max_amount = float(request.form.get('max_amount', float('inf')))

        # Filter transactions based on amount range
...
def total_balance():
    ...
    total_balance_value = sum(transaction['amount'] for transaction in transactions)
